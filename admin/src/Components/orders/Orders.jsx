...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://cooker.onrender.com/api/v1/admin-order');
            ...
            <React.Fragment key={order._id}>
              {order.product.map((product, index) => (
                <tr key={`${order._id}_${index}`}>
                  {index === 0 ? (
                    <>
                      <td className="px-6 py-4 whitespace-nowrap" rowSpan={order.product.length}><Link to={`/Orders/${order._id}`}>{order._id}</Link></td>
