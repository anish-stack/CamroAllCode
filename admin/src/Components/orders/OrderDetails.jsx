...
const OrderDetails = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const response = await axios.get(`https://cooker.onrender.com/api/v1/single-order/${id}`);
  ...
  const handleStatusUpdate = async () => {
    try {
      const res = await axios.post(`https://cooker.onrender.com/api/v1/update-order`, {
        status: newStatus,
        orderId: id
      });
  ...
  const handlePrint = async (id) => {
    try {
      const res = await axios.get(`https://cooker.onrender.com/api/v1/finduserbyid/${id}`);
